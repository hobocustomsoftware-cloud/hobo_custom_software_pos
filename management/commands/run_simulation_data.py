...
Run from Django project:  cd WeldingProject  &&  python manage.py run_simulation_data
(Canonical implementation: WeldingProject/core/management/commands/run_simulation_data.py)
"""
import random
...
class Command(BaseCommand):
    ...
    def log(self, msg, style='HTTP_INFO'):
        out = _safe_str(msg)
        self.stdout.write(getattr(self.style, style, self.style.HIGHLIGHT)(out))
    ...
    def handle(self, *args, **options):
        ...
        self.log('run_simulation_data: 60 days shop data', 'MIGRATE_HEADING')
        self.log('========================================', 'MIGRATE_HEADING')

        if dry_run:
            self.log('\n[DRY RUN] No data will be written.', 'WARNING')
            self.log('  Min transactions: %s' % min_tx, 'HTTP_INFO')
            return
        ...
        license_activated_date = base_date + timedelta(days=20)
        ...
        self.log('  Exchange rate log: 16 days (25-40)', 'SUCCESS')

        # --- Transactions: 5000+ over 60 days ---
        self.log('\n[8] Transactions (min %s)...' % min_tx, 'WARNING')
        per_day = max(84, (min_tx // 60) + 1)
        total_created = 0
        bundled_count = 0
        ...
        for day in range(1, 61):
            ...
            for _ in range(n_sales):
                ...
                if SaleTransaction.objects.filter(invoice_number=invoice_number).exists():
                    invoice_number = 'INV-SIM-%02d-%05d' % (day, total_created + 1)
            ...
            if day % 10 == 0:
                self.log('  Day %s/60 done (total %s)' % (day, total_created), 'SUCCESS')
