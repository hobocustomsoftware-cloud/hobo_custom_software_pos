  ...
  <div class="auth-page">
    <div class="auth-card">
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        <div>
          <label class="auth-label block mb-1 font-medium text-[#000000]">{{ t.phoneRequired }}</label>
        ...
        <div>
          <label class="auth-label block mb-1 font-medium text-[#000000]">{{ t.emailOptional }}</label>
          <input v-model="form.email" type="email" class="auth-input auth-input-text w-full" :placeholder="locale.isEn ? 'you@example.com (optional)' : 'you@example.com (ရွေးချယ်မှု)'" autocomplete="email" />
        </div>
        <div>
          <label class="auth-label block mb-1 font-medium text-[#000000]">{{ t.shopName }}</label>
          <input v-model="form.shop_name" type="text" class="auth-input auth-input-text w-full" :placeholder="locale.isEn ? 'My Store' : 'ဆိုင်အမည်'" />
        </div>
        <div>
...
async function handleRegister() {
  ...
  try {
    ...
    if (data?.can_login_now && data?.access) {
      ...
      const { useAuthStore } = await import('@/stores/auth')
      const authStore = useAuthStore()
      const savedLogin = (data.phone_number || '').trim() || (data.email || '').trim().toLowerCase()
      if (savedLogin) {
        try {
    ...
    if (!e.response || e.response.status === 0 || e.code === 'ERR_NETWORK') {
      ...
      if (d?.email) error.value = first(d.email)
      else if (d?.phone_number) error.value = first(d.phone_number)
