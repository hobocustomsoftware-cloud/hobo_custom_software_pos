...
const openAddModal = () => {
  isEdit.value = false
  form.value = { username: '', password: '', role: '', assignment_type: 'fixed', primary_location: null, assigned_locations: [] }
  showModal.value = true
}

// ၅။ Create / Update — backend expects: username, role_obj (pk), assignment_type, primary_location, assigned_locations, password (create only)
const saveUser = async () => {
  ...
  try {
    ...
    const msg = typeof data === 'string'
      ? data
      : data?.username?.[0] || data?.primary_outlet?.[0] || data?.detail || data?.error || (data && JSON.stringify(data)) || error.message
