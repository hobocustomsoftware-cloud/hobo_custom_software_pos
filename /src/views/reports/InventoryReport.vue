<template>
  <div class="flex-1 p-4 md:p-6 space-y-6 max-w-[1600px] mx-auto">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-black tracking-tight text-slate-900">ပစ္စည်းစာရင်း အစီရင်ခံစာ</h1>
        <p class="text-xs font-bold text-slate-500 uppercase tracking-wider mt-1">Stock Analysis</p>
      </div>
      <button
        @click="fetchData"
        :disabled="loading"
...
const stockClass = (val, row) => {
  ...
  return qty <= 0 ? 'text-rose-600 font-bold' : 'text-slate-700'
}
