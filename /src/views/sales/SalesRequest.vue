  ...
  <div class="pos-layout flex flex-col flex-1 min-h-0 w-full max-w-full -m-4 md:-m-6 p-4 md:p-6" :class="posLang === 'my' ? 'font-myanmar pos-text' : ''">
    ...
    <div class="flex md:hidden gap-2 mb-4 border-b border-border pb-2 shrink-0">
      <button
        ...
        :class="mobilePosTab === 'products' ? 'bg-[#1078D1] text-white border-[#1078D1]' : 'bg-bg border-border text-fg'"
      ...
      <button
        ...
        :class="mobilePosTab === 'cart' ? 'bg-[#1078D1] text-white border-[#1078D1]' : 'bg-bg border-border text-fg'"
      ...
      <div class="pos-products-header flex flex-wrap items-center justify-between gap-2 sm:gap-3 mb-3 md:mb-4 shrink-0">
        <div class="flex items-center gap-2 md:gap-3 flex-wrap min-w-0">
            ...
            <button
              v-if="offlinePos.isOffline"
              type="button"
              class="text-xs font-bold px-3 py-1.5 rounded-lg bg-[#1078D1] text-white border border-[#1078D1] hover:bg-[#0d62a8]"
          ...
          <div class="flex gap-2 flex-wrap items-center min-w-0 flex-1 md:flex-initial md:min-w-[280px]">
            <div class="flex gap-1 shrink-0">
              <button
                type="button"
                :class="posLang === 'my' ? 'bg-[#1078D1] text-white border-[#1078D1]' : 'bg-white text-black border-black hover:bg-gray-100'"
                class="min-h-[48px] px-3 rounded-[10px] border-2 text-sm font-bold"
                @click="posLang = 'my'"
              >MYA</button>
              <button
                type="button"
                :class="posLang === 'en' ? 'bg-[#1078D1] text-white border-[#1078D1]' : 'bg-white text-black border-black hover:bg-gray-100'"
                class="min-h-[48px] px-3 rounded-[10px] border-2 text-sm font-bold"
                @click="posLang = 'en'"
              >EN</button>
            </div>
            <div class="relative flex-1 min-w-0 w-full md:min-w-[200px]">
              ...
              <input
                ...
                ref="searchInput"
                type="text"
                :placeholder="posLang === 'my' ? 'ဘားကုဒ်/ပစ္စည်းရှာပါ...' : 'Search by barcode or name...'"
        ...
        <div class="flex flex-wrap gap-2 mb-3 md:mb-4 shrink-0">
          <button
            ...
            :class="selectedCategoryId === null ? 'bg-[#1078D1] text-white border-[#1078D1]' : 'hover:bg-gray-100'"
          ...
          <button
            ...
            class="min-h-[48px] md:min-h-[56px] px-4 md:px-6 font-semibold rounded-[12px] border-2 text-sm md:text-base"
            style="color: #000000; border-color: #000000; background: #ffffff;"
            :class="selectedCategoryId === cat.id ? 'bg-[#1078D1] text-white border-[#1078D1]' : 'hover:bg-gray-100'"
        ...
        >
          <div
            ...
            :key="p.id"
            @click="!isExpired(p) && onProductTap(p)"
            class="pos-product-card pos-product-card-span rounded-[12px] overflow-hidden transition-all border-2 flex flex-col"
            :class="[
              posLang === 'my' ? 'pos-text' : '',
              isExpired(p) ? 'opacity-60 cursor-not-allowed border-gray-300 bg-gray-50' : 'cursor-pointer border-black bg-white hover:border-[#1078D1] hover:shadow-md'
            ]"
            style="color: #000000;"
          >
            <div class="aspect-square bg-gray-100 flex items-center justify-center shrink-0 relative">
              <img
                v-if="p.image_url"
                :src="p.image_url"
                :alt="p.name"
              ...
              <span v-if="isExpired(p)" class="absolute inset-0 flex items-center justify-center bg-red-900/40 text-white text-xs font-bold rounded-b">{{ posLang === 'my' ? 'သက်တမ်းကုန်ပြီး' : 'Expired' }}</span>
            </div>
            <div class="p-3 flex flex-col flex-1 min-w-0">
              <p class="font-semibold truncate text-xs opacity-80">{{ p.sku || '—' }}</p>
              <h3 class="font-bold mt-1 line-clamp-2 text-sm leading-tight" style="color: #000000;">{{ p.name }}</h3>
              <p class="font-bold mt-2 text-sm">{{ formatMmk(itemDisplayPrice(p)) }} {{ posLang === 'my' ? 'ကျပ်' : 'MMK' }}</p>
              <p class="mt-1 text-xs opacity-80">{{ posLang === 'my' ? 'လက်ကျန်' : 'Stock' }}: {{ p.total_stock ?? p.shop_stock ?? 0 }}</p>
              <p v-if="p.expiry_date && !isExpired(p) && isExpiringSoon(p)" class="mt-1 text-xs font-semibold text-amber-600">{{ posLang === 'my' ? 'သက်တမ်းကုန်ခါနီး' : 'Expiring soon' }}</p>
            </div>
          </div>
        </div>
      </div>

    <!-- Right (40%): Order summary / Cart -->
    <div
      class="pos-ticket flex flex-col bg-white rounded-[12px] border-2 border-black p-4 sm:p-6"
      :class="mobilePosTab === 'cart' ? 'block' : 'hidden md:flex'"
    >
      <h2 class="a11y-section-title mb-4" style="color: #000000;">အရောင်းဘောက်ချာ</h2>
      ...
      <div class="flex-1 min-h-0 overflow-y-auto space-y-3 mb-4 pr-1 custom-scrollbar">
        <div
          ...
          :key="index"
          class="p-3 bg-bg rounded-[12px] border border-border flex justify-between items-center"
        >
          <div class="min-w-0 flex-1">
            <p class="font-semibold text-fg truncate pos-text">{{ item.name }}</p>
            <p v-if="item.serial_number" class="text-fg-muted text-[20px]">{{ item.serial_number }}</p>
            <div class="flex items-center gap-2 mt-1 flex-wrap">
              <button type="button" class="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg border border-border text-fg hover:bg-gray-100" @click="updateQty(index, -1)">−</button>
              <span class="min-w-[2rem] text-center font-bold pos-text">{{ formatQty(item.quantity) }}</span>
              <span class="text-fg-muted text-sm font-medium">{{ cartItemUnitLabel(item) }}</span>
              <button type="button" class="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg border border-border text-fg hover:bg-gray-100" @click="updateQty(index, 1)">+</button>
            </div>
          </div>
          <div class="flex items-center gap-2 shrink-0">
            <p class="font-bold pos-text text-fg">{{ formatMmk(itemUnitPrice(item) * item.quantity) }} ကျပ်</p>
            <button type="button" class="min-w-[40px] min-h-[40px] flex items-center justify-center text-rose-600 hover:bg-rose-50 rounded-lg" @click="removeFromCart(index)" aria-label="ဖယ်ရန်">
              <X class="w-10 h-10" />
            </button>
          </div>
        </div>
      ...
      <div class="ticket-footer border-t border-border pt-4 space-y-2">
        ...
        <div class="flex justify-between font-semibold pos-text text-fg-muted">
          <span>စုစုပေါင်း</span>
          <span>{{ formatMmk(subtotal) }} ကျပ်</span>
        </div>
        <div class="flex justify-between font-bold pos-text text-fg">
        ...
        <template v-if="!generatedInvoiceNo">
          <div v-if="paymentMethods.length > 0" class="payment-buttons grid grid-cols-2 sm:grid-cols-3 gap-2 mt-3">
            <button
              ...
              :key="pm.id"
              type="button"
              class="pos-btn min-h-[80px] leading-myanmar payment-method-btn text-base md:text-lg"
              :class="paymentButtonClass(pm)"
              :disabled="cart.length === 0 || loading"
              @click="openCheckout(pm)"
            >
              <span class="pos-text font-bold">{{ paymentButtonLabel(pm.name) }}</span>
            </button>
          </div>
          <button
            v-else
            type="button"
            class="pos-btn pos-btn-pay w-full flex items-center justify-center gap-3 leading-myanmar mt-3"
            disabled
          >
            <span class="text-[25px] font-bold">ငွေပေးချေမှုနည်းလမ်း မရှိပါ</span>
          </button>
        </template>
        <template v-else>
          <button type="button" class="pos-btn pos-btn-pay w-full flex items-center justify-center gap-2 mb-2 leading-myanmar" @click="printReceipt">
            <Printer class="a11y-icon" /> ဘောက်ချာပြန်ရိုက်မည်
          </button>
  ...
  <div v-if="showCheckoutModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50" @click.self="closeCheckout">
    <div class="bg-bg-card border border-border rounded-2xl shadow-xl max-w-md w-full p-6 pos-text" @click.stop>
      ...
      <div class="flex gap-3">
        ...
        <button
          type="button"
          class="flex-1 min-h-[56px] font-bold rounded-xl bg-[#1078D1] text-white hover:opacity-90 disabled:opacity-50"
...
import { Search, X, Printer, ScanLine } from 'lucide-vue-next'
...
const updateQty = (index, delta) => {
  ...
  // Shopfloor only: cap quantity by shop_stock at selected sale location
  const stock = product ? (product.shop_stock ?? product.total_stock ?? 0) : 0
  const factor = getConversionFactor(item.unit)
...
// --- Unit price: use exchange rate for DYNAMIC_USD products; hardware mode = base price × conversion factor ---
...
const submitSaleRequest = async () => {
  ...
  try {
    ...
    if (result.ok && result.offline) {
      ...
      alert('အရောင်းဘောက်ချာ တင်သွင်းမှု အောင်မြင်ပါသည်')
    } else {
...
function paymentButtonClass(pm) {
  ...
  if (n.includes('cash') || n === 'ငွေသား') return 'pos-btn-pay'
...
/* Responsive product grid: tablet 6 cols, desktop 12 cols (sale လုပ်ဖို့ အဆင်ပြေအောင်) */
.pos-product-grid {
  display: grid;
...
/* Tablet: 6 columns → each card spans 2 = 3 products per row */
@media (min-width: 768px) {
  .pos-product-grid {
...
/* Desktop: 12 columns → each card spans 2 = 6 products per row */
