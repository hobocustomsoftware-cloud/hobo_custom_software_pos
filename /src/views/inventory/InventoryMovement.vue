<template>
  <div class="space-y-6 bg-[#f4f4f4] min-h-full">
    <div class="flex flex-wrap justify-between items-center" style="gap: var(--fluid-gap);">
      <h1 class="text-fluid-xl font-black uppercase tracking-tight text-[#1a1a1a]">Stock Movement</h1>
    ...
    >
      <template #cell-created_at="{ value }">
        <span class="text-[#4b5563] text-fluid-sm">{{ formatDate(value) }}</span>
      </template>
      <template #cell-product_name="{ value }">
        <span class="font-semibold text-[#1a1a1a] uppercase text-fluid-sm">{{ value }}</span>
      </template>
      <template #cell-from_location_name="{ row }">
        <div class="flex flex-col gap-0.5">
          <span class="text-[10px] font-bold text-[#6b7280] uppercase">{{ row.to_location_name ? 'Path' : 'Inbound' }}</span>
          <div class="flex items-center gap-1.5 text-fluid-sm">
            <span class="text-[#4b5563]">{{ row.from_location_name || '📦 NEW' }}</span>
            <ArrowRight v-if="row.to_location_name" class="w-3.5 h-3.5 text-[#9ca3af]" />
            <span class="text-[#1a1a1a]">{{ row.to_location_name || '' }}</span>
          </div>
        </div>
      </template>
    ...
    <div v-if="showAddModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div class="bg-white w-full max-w-2xl p-8 relative z-10 rounded-xl border border-[var(--color-border)] shadow-xl">
        <button
          ...
          class="absolute top-4 right-4 p-2 rounded-xl text-[#6b7280] hover:text-[#1a1a1a] hover:bg-[var(--color-bg-card)] transition"
        >
          <X class="w-5 h-5" />
        </button>
        <h2 class="text-fluid-xl font-black text-[#1a1a1a] mb-6 uppercase tracking-tight">Stock Transfer</h2>
