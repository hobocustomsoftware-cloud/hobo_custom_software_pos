...
// Menu Items Categorized for Sidebar
const generalItems = computed(() =>
  filteredMenu.value.filter((i) =>
    ['Dashboard', 'Sales Request', 'Inventory', 'Sales History', 'Service', 'Approve', 'Settings'].includes(
      i.name,
    ),
  ),
)
...
const reportItems = computed(() => {
  ...
  const canViewReports = ['super_admin', 'owner', 'manager', 'assistant_manager', 'inventory_manager', 'sale_supervisor'].includes(role)
