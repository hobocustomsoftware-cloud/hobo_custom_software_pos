...
class ProductListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        assigned_locs = user.assigned_locations.filter(outlet_id=outlet_id)
        if not assigned_locs.exists():
            return Product.objects.none()

        from core.models import StaffSession
        session = user.work_sessions.filter(is_active=True).first()
        location = session.location if session else user.primary_location
...
class InventoryMovementListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return filter_queryset_by_outlet(
            InventoryMovement.objects.all().select_related('product', 'from_location', 'to_location', 'moved_by').order_by('-created_at'),
            self.request,
        )
...
class InventoryInboundView(APIView):
    ...
    def post(self, request):
        try:
            ...
            log_audit(request.user, 'movement_inbound', 'InventoryMovement', movement.id, outlet_id=to_loc.outlet_id, details={'product_id': product_id, 'quantity': int(qty)})
            return Response({
...
class InventoryTransferView(APIView):
    ...
    def post(self, request):
        try:
            ...
            log_audit(request.user, 'movement_transfer', 'InventoryMovement', movement.id, outlet_id=outlet_id, details={'product_id': product_id, 'quantity': int(qty)})
...
class InvoiceListView(generics.ListAPIView):
    """Manager+ only. Cashier cannot see full invoice list (use staff sale history).
    Query params: date_from (YYYY-MM-DD), date_to (YYYY-MM-DD), outlet_id – for per-shop monthly reports."""
    serializer_class = InvoiceSerializer
    permission_classes = [IsManagerOrHigher]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter, DjangoFilterBackend]
    search_fields = ['invoice_number', 'customer__name', 'customer__phone_number']
...
class DashboardAnalyticsView(APIView):
    """Manager+ only. Cashier/Store Keeper get 403 (reports)."""
    permission_classes = [IsManagerOrHigher]

    def get(self, request):
        try:
            user = request.user
            role_name = user.role_obj.name.lower() if user.role_obj else "staff"
            ...
            data = {
                ...
                "recent_activities": self.get_recent_activities(user, role_name, outlet_id)
            }
            return Response(data)
        except Exception as e:
    ...
    def get_stats_by_role(self, user, role_name, low_stock_count, start_date, single_day=False, outlet_id=None):
        ...
        active_repairs = RepairService.objects.exclude(status__in=['completed', 'cancelled']).count()
        ...
        # Owner/Admin: ဒီနေ့ အရောင်းစာရင်း၊ profit, loss, margin ပြချက်
    ...
    def get_charts_by_role(self, user, role_name, start_date=None, single_day=False, outlet_id=None):
        ...
        service_f = Q()
        loc_sales_f = Q(saletransaction__status='approved')
        if outlet_id is not None:
            loc_sales_f &= Q(saletransaction__outlet_id=outlet_id)
        ...
        if start_date:
            if single_day:
                ...
                loc_sales_f &= Q(saletransaction__created_at__date=start_date)
                ...
                loc_sales_f &= Q(saletransaction__created_at__date__gte=start_date)
                loc_service_f &= Q(repairs__created_at__date=start_date)
        ...
        branch_performance = locs_qs.annotate(
            s_rev=Coalesce(Sum('saletransaction__total_amount', filter=loc_sales_f), 0.0, output_field=models.FloatField()),
            sv_rev=Coalesce(Sum('repairs__total_estimated_cost', filter=loc_service_f), 0.0, output_field=models.FloatField()),
            s_cnt=Count('saletransaction', filter=loc_sales_f),
    ...
    def get_recent_activities(self, user, role_name, outlet_id=None):
        """Table အတွက် Activities (outlet-scoped for non-owner)."""
        if not is_owner(user):
            outlet_id = getattr(user, "primary_outlet_id", None)
        activities = []
        ...
        for s in sales_qs:
            activities.append({
                "id": s.id, "time": s.created_at, "category": "Sales",
                "message": f"Sales #{s.invoice_number} is {s.status}",
                "user": s.staff.username if s.staff else "System", "status": s.status.capitalize()
            })

        repairs = RepairService.objects.all().select_related('staff').order_by('-created_at')[:5]
        for r in repairs:
            activities.append({
                "id": r.id, "time": r.created_at, "category": "Service",
                "message": f"Repair {r.repair_no} is {r.status}",
                "user": r.staff.username if r.staff else "Technician", "status": r.status.capitalize()
            })
...
class PaymentMethodViewSet(viewsets.ModelViewSet):
    """Payment Method CRUD - Owner/Admin only"""
...
class StockPredictionView(APIView):
    ...
    def get(self, request):
        ...
        predictions.sort(key=lambda x: x['days_until_out_of_stock'] or 999)
