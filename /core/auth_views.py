...
class UnifiedLoginView(APIView):
    """
    POST: Login with a single field (Phone or Email) + Password.
    Body: { "login": "09xxxxxxxx or user@example.com", "password": "...", "country_code": "+95" (optional) }
    Validation: if login starts with 09 or +95 → phone; else → email.
    Returns: { "access", "refresh", "user": {...}, "outlet": {...} or null }
    After 5 failed attempts per phone/email, returns 403 locked.
    """
    ...
    def post(self, request):
        ...
        if normalized_phone:
            ...
            email_val = lockout_key[2:] if lockout_key.startswith("e:") else lockout_key  # strip "e:"
            user = qs.filter(email__iexact=email_val).first()
            not_found_detail = "No active account found with this email address."
        ...
        try:
            # Re-fetch user with select_related so role_obj/primary_outlet are loaded (avoid 500 from lazy access)
            user = User.objects.filter(pk=user.pk).select_related("role_obj", "primary_outlet").first()
            if not user:
            ...
            import logging
            logging.getLogger(__name__).exception("Login response build failed: %s", e)
    ...
    def post(self, request):
        ...
        if login_raw:
            ...
            if normalized_phone:
                ...
                email_val = lockout_key[2:] if lockout_key.startswith("e:") else lockout_key
                user = User.objects.filter(email__iexact=email_val, is_active=True).first()
