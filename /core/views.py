...
from .models import User, Role, StaffSession, ShopSettings, Outlet
from .serializers import UserSerializer, EmployeeSerializer, RoleSerializer, RegisterSerializer, ShopSettingsSerializer
from .permissions import IsAdminOrHigher, IsOwner, IsStaffOrHigher
from .outlet_utils import get_visible_outlets
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if 'unique' in err_msg.lower() or 'duplicate' in err_msg.lower():
                return Response({
                    'email': ['This email or phone is already registered. Please login.'],
        ...
        msg = 'Registration successful. You can sign in now.' if is_first else 'Registration successful. Admin must approve before you can sign in.'
        payload = {
            'message': msg,
            'email': getattr(user, 'email', None) or '',
            'phone_number': getattr(user, 'phone_number', None) or '',
            'username': user.username,
            'can_login_now': is_first,
        }
        # Loyverse-style: first user gets tokens so frontend can go to setup-wizard without a second login (avoids 401)
        if is_first:
...
class RoleViewSet(viewsets.ModelViewSet):
    """Owner စိတ်ကြိုက် Role များ CRUD လုပ်ရန် API"""
...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return User.objects.prefetch_related('assigned_locations').select_related('role_obj', 'primary_location').order_by('-id')
...
class ShopSettingsView(APIView):
    ...
    def check_put_allowed(self, request):
        """Allow PUT for Owner, first user, or any authenticated user when setup not done yet (Loyverse-style)."""
        if not request.user or not request.user.is_authenticated:
            return False
        if request.user.is_superuser:
            return True
        if getattr(request.user, 'role_obj', None) and 'owner' in (request.user.role_obj.name or '').lower():
            return True
