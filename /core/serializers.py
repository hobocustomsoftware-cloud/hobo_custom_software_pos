...
from .models import User, Role, ShopSettings, BUSINESS_CATEGORY_CHOICES, CURRENCY_CHOICES
...
class UserSerializer(serializers.ModelSerializer):
    role_name = serializers.ReadOnlyField(source='role_obj.name')
    ...
    def get_primary_outlet_info(self, obj):
        if obj.primary_outlet_id:
            return {'id': obj.primary_outlet.id, 'name': obj.primary_outlet.name, 'code': obj.primary_outlet.code or ''}
        return None

    def get_current_location(self, obj):
        """ယနေ့အလုပ်လုပ်မည့်ဆိုင် (StaffSession မှ ယူသည်)"""
        session = obj.work_sessions.filter(is_active=True).first()
        if session:
            return {'id': session.location.id, 'name': session.location.name}
        if obj.primary_location:
            return {'id': obj.primary_location.id, 'name': obj.primary_location.name}
        loc = obj.assigned_locations.filter(is_sale_location=True).first()
        if loc:
    ...
    def get_assigned_locations_list(self, obj):
        ...
        return [{'id': loc.id, 'name': loc.name} for loc in obj.assigned_locations.filter(is_sale_location=True)]
